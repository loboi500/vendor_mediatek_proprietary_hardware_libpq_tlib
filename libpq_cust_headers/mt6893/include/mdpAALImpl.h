...
struct DREInitParam {
    ...
    int demo_win_y_end;
//#if (CALTM_VERSION == CALTM_VERSION_3_5)
#if (CALTM_VERSION != CALTM_VERSION_3_0)
...
#endif
#if (CALTM_VERSION == CALTM_VERSION_3_5)
    int fw_ctrl_dre_s_lower2;
    int fw_ctrl_dre_s_upper2;
    int fw_ctrl_dre_y_lower2;
    int fw_ctrl_dre_y_upper2;
    int fw_ctrl_dre_h_lower2;
    int fw_ctrl_dre_h_upper2;
    int fw_ctrl_dre_s_lower3;
    int fw_ctrl_dre_s_upper3;
    int fw_ctrl_dre_y_lower3;
    int fw_ctrl_dre_y_upper3;
    int fw_ctrl_dre_h_lower3;
    int fw_ctrl_dre_h_upper3;
    int fw_ctrl_bilateral_flt_en;
    int fw_ctrl_bilateral_impulse_noise_en;
    int fw_ctrl_bilateral_range_flt_slope;
    int fw_ctrl_dre_bilateral_blending_wgt;
//#ifdef Face_Adjsutment_Top
    int face_brightness_compensation_th;
    int face_brightness_compensation_slope;
    int face_brightness_compensation_min;
    int face_brightness_compensation_max;
    int face_contrast_enhance_th;
    int face_contrast_enhance_slope;
    int face_contrast_enhance_min;
    int face_contrast_enhance_max;
//#endif // Face_Adjsutment_Top
    int fw_ctrl_dre_h_slope;
    int fw_ctrl_dre_s_slope;
    int fw_ctrl_dre_y_slope;
    int fw_ctrl_dre_h_slope2;
    int fw_ctrl_dre_s_slope2;
    int fw_ctrl_dre_y_slope2;
    int fw_ctrl_dre_h_slope3;
    int fw_ctrl_dre_s_slope3;
    int fw_ctrl_dre_y_slope3;
    int fw_ctrl_dre_bilateral_blending_en;
    int fw_ctrl_dre_bilateral_detect_en;
#endif
};
...
#ifdef Face_Adjsutment_Top
struct Face_Input {
    uint32_t LCE_face_control_en;
    uint32_t Face_DR_weight;
    uint32_t FDY;
    uint32_t Face_Comp_th;
    uint32_t Adaptive_APLComp_en;
    uint32_t AAPLC_ThL_Low;
    uint32_t AAPLC_ThH_Low;
    uint32_t AAPLC_Max_Low;
    uint32_t AAPLC_Min_Low;
    uint32_t AAPLC_ThL_High;
    uint32_t AAPLC_ThH_High;
    uint32_t AAPLC_Max_High;
    uint32_t AAPLC_Min_High;
};
#endif // Face_Adjsutment_Top
struct DRETopInput {
...
#elif (CALTM_VERSION == CALTM_VERSION_3_5)
    int DREChromaHist[mdpDRE_BLK_NUM_Y][mdpDRE_BLK_NUM_X];
    int DREChromaHist2[mdpDRE_BLK_NUM_Y][mdpDRE_BLK_NUM_X];
    int DREChromaHist3[mdpDRE_BLK_NUM_Y][mdpDRE_BLK_NUM_X];
#endif
...
#if (CALTM_VERSION == CALTM_VERSION_3_5)
    int LocalAPLRatio[128];
#ifdef Face_Adjsutment_Top
    struct Face_Input Face_Input;
#endif // Face_Adjsutment_Top
#endif
};
...
#if (CALTM_VERSION == CALTM_VERSION_3_5)
#ifdef Face_Adjsutment_Top
  void L_AdaptiveLuma_Block(int dre_blk_x_num, int dre_blk_y_num, int HistNorEn, int *HisBin, int RGBMaxSum, int ChromaHistCount, int *iYFtn, int ChromaHistCount2, uint32_t LCE_face_control_en, uint32_t Face_DR_weight, uint32_t FDY, uint32_t Face_Comp_th, uint32_t Adaptive_APLComp_en, uint32_t AAPLC_Max_High, uint32_t AAPLC_Min_High, uint32_t AAPLC_ThL_High, uint32_t AAPLC_ThH_High, uint32_t AAPLC_Max_Low, uint32_t AAPLC_Min_Low, uint32_t AAPLC_ThL_Low, uint32_t AAPLC_ThH_Low);
#else
...
#if (CALTM_VERSION == CALTM_VERSION_3_5)
  void DualPipeBlockStatistics(const DRETopInput Pipe1, const DRETopInput Pipe2, const int HistSumPipe1[16], const int HistSumPipe2[16], int Pipe2_x_start, DRETopInput &Full);
#endif
