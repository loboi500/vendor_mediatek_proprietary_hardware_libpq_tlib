...
enum ADLDebugFlags {
    ...
    eADLDebugEntryFlag         = 0x1,
    eADLDebugInput             = 0x2,
    eADLDebugOutput            = 0x4,
    eADLDebugTime              = 0x8,
    eADLDebugContent           = 0x10,
    eADLDebugAll               = 0xFF
};
...
struct DualInfoDync{
       ...
       int hist_sum_init_pipe0;
       int hist_sum_init_pipe1;
       int contour_hist_init_pipe0[17];
       int contour_hist_init_pipe1[17];
       int hist_bin_init_pipe0[17];
       int hist_bin_init_pipe1[17];
};



// =================== feedback parameters per frame for Async Bliter case ================== //
struct DCsharedata
{
    uint32_t preTargetCurve[LUMA_CURVE_NUM]; // = waCurCurve
    uint32_t preFinalCurve[LUMA_CURVE_NUM];  // = waLumaCurve
};

struct DCPreTempData
{
    //#if DYN_CONTRAST_VERSION == 2
    ...
    uint32_t waTCurvePre[LUMA_CURVE_NUM]; // = preFinalCurve
    uint32_t wHistPre[LUMA_HIST_NUM_WEIGHT]; // Acctually is uint16_t
    uint32_t u4PreAPL;
...
private:
  int session_switch; // counter for async bliter case.
...
public:
    ...
    ADLReg * pHDRModeReg;
    DCsharedata * pDCsharedata;
    void initDCShareMemory(void);
    void setShareMemory(DCsharedata *sharedata);
    void getShareMemory(DCsharedata *sharedata);
    void setDCPreTempData(DCPreTempData *previousTempData);
    void getDCPreTempData(DCPreTempData *previousTempData);
...
void refreshOutput(ADLOutput *output, DCsharedata *sharedata);
void combineDyncDualPipeInput(ADLInput & pipe0, ADLInput & pipe1, ADLInput & full, DualInfoDync &pipeinfo, const int wholefbwidth, const int wholefbheight, const int srcwidth, const int srcheight);
private:
    uint32_t DebugFlags;
