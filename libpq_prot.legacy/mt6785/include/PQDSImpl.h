...
#define VER_6771  2
  ...
  #define DYN_SHARP_VERSION VER_6771
  ...
  #ifdef PQ_TDSHP_VERSION_0
    #define HFG_VERSION_6885
    #define DYN_SHARP_VERSION VER_6771
  #endif
#endif
   ...
   //#define DSHP_TRANSITION_SUPPORT 1
   //#define TEST_DSHP_TRANSITION 1
   //#define TEST_DSHP_AIPQ 1
#endif
...
struct DSSWCustXMLBuf
{
  unsigned int Size;//Byte
  DSReg iDSSWRegXMLEntry;
...
//#ifdef DSHP_TRANSITION_SUPPORT
enum {
    ENUM_SharpToSmooth,
    ENUM_SmoothToSharp
};

struct DSTransitionRegs {
    uint32_t tdshp_gain_mid;
    uint32_t tdshp_gain_high;
    uint32_t tdshp_gain;
};

struct DSTransitionSettings {
    int Step;
...
public:
...
#ifdef DSHP_ANDROID_PLATFORM
    int onTransition(const DSInput *input, DSOutput *output);
    ...
    void PQ_selector( IN PQSelectorInput pqSelectorInput, OUT int &TDSHP_sel);
    void AIPQ_selector( const float NNSceneProbability[15], float (&AALSceneProbability)[5], const int wholefbwidth, const int wholefbheight, const int srcwidth, const int srcheight, \
        const int QPsum, int &TDSHP_sel);

//#ifdef DSHP_TRANSITION_SUPPORT
    int InitialDSTransitionSettings(DSTransitionSettings &input, int Step, int Speed);
    int InitialDSTransitionRegs(DSTransitionRegs &input);
    int IfDiffDSTransitionRegs(const DSTransitionRegs *DSReg1, const DSTransitionRegs *DSReg2, const DSInput *input = NULL);

    int AssignDSTransitionRegs(DSTransitionRegs *Target, DSTransitionRegs *Source, const DSInput *input = NULL, const DSOutput *output = NULL);
    int TransitionRegsToOutput(DSOutput *output, DSTransitionRegs &Current);
    int ComparedDSTransitionRegs(DSTransitionRegs *DSReg1, DSTransitionRegs *DSReg2);

    int GetTransitionRegWeight(DSTransitionRegs *input);
    int ModeInterpolation(uint32_t Target, uint32_t Previous, int CurrentStep, int TotalStep);
    int onTransition(const DSTransitionSettings &TransitionSettings);
    int onTransition();
    int EnableDSHPTransition(bool enable);
    void SetSmoothToSharpStep(int Step);
    void SetSharpToSmoothStep(int Step);
    int GetSmoothToSharpStep();
    int GetSharpToSmoothStep();
//#endif

    CPQDSFW();
    CPQDSFW(bool disp_tdshp);
    ~CPQDSFW();
...
private:
    ...
    DSReg * pDSRegOld;
//#ifdef DSHP_TRANSITION_SUPPORT
    DSTransitionSettings SmoothToSharpDSTransition;
    DSTransitionSettings SharpToSmoothDSTransition;
    DSTransitionRegs TargetRegs;
    DSTransitionRegs PreviousRegs;
    DSTransitionRegs CurrentRegs;
    int CurrentStep;
    int CurrentCount;
    int CurrentSmoothSharpMode;  //0: Sharp to Smooth, 1: Smooth to Sharp
    int Finished;     //To check if transition is finish yet
    bool DSHP_transition_enable;
//#endif
    int CurrentTDSHPTable;
    bool TraceFlag;
    int SHPStrength;
    int SHPStepValue;
    int URStrength;
    bool DISP_TDSHP_enable;

#ifdef TEST_DSHP_TRANSITION
    uint32_t tmp_gain_mid;
#endif
    ...
    // Debug logger
    void logMechanism(const DSInput*);
