...
#if defined(PQ_HDR_VERSION_0) || defined(PQ_HDR_VERSION_1)
    #define HDR_VERSION_1
#endif

#ifdef HDR_VERSION_1
    #define before_tone_map_linear_bits 22
    #define after_tone_map_linear_bits 22
#endif
...
#ifdef HDR_VERSION_1
    #define Gain_Curve_POINTS 1024
    #define Gain_Curve_INT_BITS 7
    #define Gain_Curve_FLT_BITS 16
#endif
...
#ifndef Gain_Curve_POINTS
    #define Gain_Curve_POINTS 1024
#endif

#ifndef Gain_Curve_INT_BITS
    #define Gain_Curve_INT_BITS 7
#endif

#ifndef Gain_Curve_FLT_BITS
    #define Gain_Curve_FLT_BITS 16
#endif
...
#ifdef HDR_VERSION_1
    #define Y2R_Progarmmalbel_Y2R_BT2020C 4
#endif
...
#ifdef HDR_VERSION_1
    #define HDR10_PLUS_PROJECT //Based on TV_PROJECT
    #define __MAIN_PATH_ONLY__ //diable for SUB Path instance
#endif

//#define _HDR10_PLUS_ //support HDR1

#ifdef HDR_VERSION_1
    #define HLG_SYSTEM_Beta
#endif

#define MOBILE_PROJECT

//#ifdef HDR_VERSION_1
    #define LCE_APPLICATIOM
    #define HDR10_PLUS_Profile_LEVEL 0   // 0: profile A, 1: profile B
    #define Num_HDRInternalReg_Dump_cmparam_pHDRFWReg 161
    #define Num_HDRInternalReg_Dump_cmparam_DHDRINPUT 286
    #define Num_HDRInternalReg_Dump_cmparam_DHDROUTPUT 1305
    #define Num_HDRInternalReg_Dump_cmparam (Num_HDRInternalReg_Dump_cmparam_pHDRFWReg + Num_HDRInternalReg_Dump_cmparam_DHDRINPUT + Num_HDRInternalReg_Dump_cmparam_DHDROUTPUT)
//#endif
...
enum HDRDebugFlags {
    ...
    eHDRDebugEntryFlag                      = 0x1,
    eHDRDebugInputDHDRINPUT_HDRFWReg        = 0x2,
    eHDRDebugInputFirewareMemberVariable    = 0x4,
    eHDRDebugOutputDHDROUTPUT               = 0x8,
    eHDRDebugOutputGainCurve                = 0x10,
    eHDRDebugTime                           = 0x20,
    eHDRDebugContent_Frame                  = 0x40,
    eHDRDebugContent_Stream                 = 0x80,
    eHDRDebugAll                            = 0xFF
};
...
//#ifdef HDR_VERSION_1
typedef struct aiToneMappingCurve
{
    float sw_p[9];
    float sw_slope[8];
...
//#ifdef HDR_VERSION_1
// provided by customer's application
typedef enum APPLICATION_E
{
    VIDEO           = 0,  // Video Playback
    CCORR           = 1,  // HDR as CCORR
    GAME_HDR        = 2,  // Game HDR
    AI_SDR2HDR      = 3,  // AI SDR-to-HDR
    E2E_HDR_PREVIEW = 4,  // Preview for hdr10+ record
    E2E_HDR_RECORD  = 5,  // Record for End-to-End HDR
    VIDEO_HBM       = 6,  // Video Playback for High Bright Mode
    Lite_PQ_2nd = 7,      // 2nd Display for Lite_PQ
    Main_PQ_2nd = 8,      // 2nd Display for Main_PQ
} APPLICATION_E;
//#endif

typedef enum PQ_HDR_INPUT_COLOR_SPACE
{
  PQ_HDR_YUV = 0,
  PQ_HDR_RGB,
} PQ_HDR_INPUT_COLOR_SPACE;
...
//#ifdef HDR_VERSION_1
  P3     = 2,
  Others = 3, //enum will be increased by customer's panel spec
//#endif
} GAMUT;

typedef struct PANEL_SPEC
{
  UINT32 panel_nits; //Lw
  enum GAMUT gamut;
//#ifdef HDR_VERSION_1
  UINT32 Lb;
//#endif
...
typedef struct HDRFWReg
{
  ...
  UINT32 ccorr_en;
  UINT32 Reserved_Array[100];
}HDRFWReg;

// =================== HDR SW cust registers ================== //
#pragma pack(1)
typedef struct custHDRFWReg
{
  UINT32 panel_nits;
  ...
  UINT32 Reserved_Array[50];
}custHDRFWReg;

typedef struct HDR10_custXMLBuf{
  unsigned int Size;//Byte
  HDRFWReg iHDRRegEntry;
...
//#ifdef HDR_VERSION_1
  float NNSceneProbability[15]; // for AI-SDR2HDR, Slope [7:0]
//#endif

  UINT32 currALI; // Calibrated value(LUX) from light sensor
  PQ_HDR_INPUT_COLOR_SPACE color_space;
}DHDRINPUT;
...
#pragma region GameHDR
  UINT32 y2r_en;
  UINT32 GameHDR_LCE_Sel;          // select LCE setting
  UINT32 GameHDR_Color_Sel;          // select Color setting
#pragma endregion
...
//#elif HDR_VERSION == 1
...
#else
  custHDRFWReg iHDRRegEntry[HDR_TBL_SIZE];
  HDRFWReg iHDRRegEntry_xml;
  void onLoadXML(unsigned char* setting);
  void initHDRRegEntry();
  void onInitCommon(void);
  void onInitPlatform(DHDRINPUT *input, DHDROUTPUT *output);
  void onInitPlatform(char *tableName);  // for customer setting
  void onCalculate(DHDRINPUT *input, DHDROUTPUT *output);
  void aiToneCurveRemap(IN DHDRINPUT *input, struct HDRFWMemberVariable *pHDRFWMV);
  ...
  void XMLApplication_Selector(IN APPLICATION_E Application, OUT int *HDR_sel);
  void custParamMapping(int *HDR_sel);
  void XMLCustParamMapping(IN APPLICATION_E Application);

  void setting_by_SWreg(DHDRINPUT *input,DHDROUTPUT *output, struct HDRFWMemberVariable *pHDRFWMV, struct HDRFWReg *pHDRFWReg);
...
//#ifdef HDR_VERSION_1
  void DumpRegToFile(FILE *fb, DHDRINPUT *input, DHDROUTPUT *output);
//#endif
...
//#ifdef HDR_VERSION_1
  aiToneMappingCurve paiToneMappingCurve;
...
//#ifdef HDR_VERSION_1
    extern UINT32 GainCurve_pool[25][Gain_Curve_POINTS];
//#endif
